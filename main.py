...
def ecommerce_chain(llm):
    ...
    store_prompt = PromptTemplate(
        ...
        template="Which e-commerce store is best for buying {product}? Only list a single store.",
    )

    # Second prompt: Get popular products from the store name
    products_prompt = PromptTemplate(
        input_variables=["store"],
        template="List some popular products available on {store}, separated by commas. Only list the products.",
    )
    ...
    overall_chain = SimpleSequentialChain(
        chains=[store_chain, products_chain], verbose=True
    )
